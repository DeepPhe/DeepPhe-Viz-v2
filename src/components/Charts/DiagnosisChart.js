...
export default class DiagnosisChart extends React.Component {
  ...
  updateDiagnosisData = async () => {
    ...
    const fetchData = async () => {
      ...
      return new Promise(function (resolve, reject) {
        fetch("http://localhost:3001/api/diagnosis/" + patientIds.join("+")).then(function (
          response
        ) {
          if (response) {
            resolve(response);
  ...
  showDiagnosisChart = (svgContainerId) => {
    ...
    const svgHeight =
      xAxisHeight + yAxisHeight + chartTopMargin + overviewHeight + gapBetweenYAxisAndXAxis * 2;
    // const chartWidth = svgWidth - svgPadding.left - svgPadding.right;

    const chartHeight =
      svgHeight - svgPadding.top - svgPadding.bottom - overviewHeight - gapBetweenYAxisAndXAxis;
    ...
    const overviewWidth =
      that.state.width - svgPadding.left - svgPadding.right - gapBetweenYAxisAndXAxis;
    ...
    // Set the ranges
    let x = d3.scalePoint().domain(defaultPatients).range([gapBetweenYAxisAndXAxis, overviewWidth]);

    let overviewX = d3.scalePoint().domain(xDomain).range([gapBetweenYAxisAndXAxis, overviewWidth]);
    ...
    let overviewY = d3.scalePoint().domain(data.diagnosisGroups).range([0, overviewHeight]);
    ...
    diagnosisChartGrp
      .selectAll(".diagnosis_dot")
      .data(
        diagnosisDots.filter(function (obj) {
    ...
    function createXAxis() {
      diagnosisChartGrp
        ...
        .attr(
          "transform",
          "translate(0," + (chartHeight - chartTopMargin - svgPadding.bottom) + ")"
        )
        .attr("class", "diagnosis_x_axis")
        ...
        .on("mouseover", function (d) {
          ...
          d3.select(".diagnosis_chart_group")
    ...
    function createSlider() {
      ...
      let overview = svg
        ...
        .attr(
          ...
          "translate(" +
            svgPadding.left +
            "," +
            (svgPadding.top + chartHeight + gapBetweenYAxisAndXAxis) +
            ")"
        );

      overview
        .selectAll(".overview_diagnosis_dot")
        .data(diagnosisDots)
        .enter()
        .append("g")
        .append("circle")
      ...
      function dragged(event, d) {
        ...
        const endX =
          overviewX(xDomain[xDomain.length - patientsNumDisplay]) - overviewDotRadius * 2;
