...
export default class CohortFilter extends React.Component {
  ...
  reset = () => {
    ...
    const fetchFilterDefinition = async () => {
      return new Promise(function(resolve, reject) {
        fetch('http://localhost:3001/api/filter/definitions').
            then(function(response) {
              if (response) {
                resolve(response);
              }
    ...
    const fetchPatientArraysPromise = new Promise(
        (resolve, reject) => fetchPatientArrays().then(function(response) {
    ...
    const fetchFilterDefinitionPromise = new Promise((resolve, reject) => {
      fetchFilterDefinition().then(function(response) {
        response.json().then(function(json) {
          let cohortSize = [
            {
              value: 5, description: '5', color: 'blue',
            }, {
              value: 95, description: '', color: 'lightgray',
            }];

          that.setState({
            filterDefinitions: json, cohortSize: cohortSize, isLoading: false,
          }, () => {
            resolve(json);
          });
        });
    ...
    function initFilterData() {
      return new Promise((resolve, reject) => {
        ...
        filterDefinitions.forEach((definition, i) => {
          filters[definition.fieldName] = new Filter(definition, patientArrays);
        });
    ...
    Promise.all([fetchPatientArraysPromise, fetchFilterDefinitionPromise]).
        then((promises) => {
          initFilterData().then(() => {
      ...
      let cohortSize = [
        {
          ...
          value: 7, description: '', color: 'lightgray',
        }];
      that.setState({
    ...
    async function updatePatientsMatchingAllFilters() {
      let matches = {};
      ...
      if (Object.keys(matches).length) {
        ...
        console.log('Matches across all filters: ' +
    ...
    function CohortPercentHSBar(props) {
      return <HSBar showTextIn max={100} height={47.3}
                    data={this.state.cohortSize} />;
    };

    function filterChangedState(definition) {
    ...
    {
      //only on filterDefinitionChange
      ...
      function toggleFilterEnabled(activity) {
        const selector = '#' +
            activity.filterDefinition.fieldName.replaceAll(' ', '-').
                toLowerCase() + '-overlay-row';
        if (enabled) {
      ...
      function filterAndFilterBar({filterDefinition, index}) {
      ...
      moveListItem(dragIndex, hoverIndex);
      {
        ...
        this.setState(update(this.state, {
          items: {
            $splice: [
              [dragIndex, 1], [hoverIndex, 0, dragitem]],
          },
        }));
      ...
      function handleDragEnd(result, that) {
        ...
        that.setState({handlingDragEnd: true}, () => {
          const removeAndInsert = (array, removeFrom, insertAt) => {
            ...
            return [
              ...array.slice(0, insertAt), item[0], ...array.slice(insertAt)];
          };
          console.log(
              'handleDragEnd - moving ' + result.draggableId + ' from ' +
              result.source.index + ' to ' + result.destination.index);
      ...
      render();
      {
        if (this.state.isLoading || this.state.filterDefinitionLoading ||
            this.state.patientArraysLoading ||
            this.state.handlingDragEnd) return <div>Data is coming
          soon...</div>; else return (<React.Fragment>
          <div id="NewControl">
            <Grid
                ...
                align="center"
            >
              <Grid className={'no_padding_grid cohort-size-label-container'}
                    item md={1}>
            ...
            >
              <DragDropContext
                  onDragEnd={(result) => this.handleDragEnd(result, this)}>
                <Droppable droppableId="droppable">
                  ...
                  >
                    {this.state.filters.map(
                        (filter, index) => (<this.filterAndFilterBar
                            key={index}
